...
## 2. 계정 설정 화면 (브라우저 인터페이스) ⭐ 먼저 여기서

```bat
python webui\app.py
```

또는 `setup_accounts.bat` 더블클릭 → 브라우저에서 **http://127.0.0.1:5000** 이 열립니다.

이 화면에서 할 수 있는 것:

- **채널(계정 그룹) 추가/수정/삭제** — 이름(예: 심리1, 심리2), 설명, 사용할 플랫폼 선택
- **플랫폼별 아이디·비밀번호 입력·저장** — 저장하면 자동 로그인에 사용됩니다
- **직접 로그인** 버튼 — 비밀번호 저장 대신(또는 2단계 인증·캡차가 있을 때) 브라우저 창을 띄워 직접 로그인
- 각 칸에 **로그인 정보/세션 상태**가 표시됩니다
- 기본 채널, 감시 폴더, 검사 주기 설정

> 🔒 **보안**: 입력한 비밀번호는 평문이 아니라 `credentials.enc`(암호화)에 저장되고,
> 암호화 열쇠는 `secret.key`에 보관됩니다. 두 파일은 `.gitignore`로 커밋이 막혀 있으며
> **절대 공유하지 마세요**. 이 PC에 접근할 수 있는 사람은 복호화가 가능하니 PC 자체를 안전하게 관리하세요.

> ⚠ **자동 로그인의 한계**: 페이스북·인스타·틱톡·네이버는 자동 입력 로그인 시
> **2단계 인증·캡차(슬라이드 퍼즐)·"새 기기" 확인**을 자주 요구합니다.
> 이 경우 자동 로그인이 멈추므로, 가장 안정적인 방법은 **'직접 로그인' 버튼으로 한 번
> 로그인해 세션을 저장**하는 것입니다. 한 번 저장된 세션은 계속 재사용됩니다.

## 3. 계정 그룹 등록 (수동 편집): `config.json`

> 위 설정 화면을 쓰면 이 파일을 직접 건드릴 필요가 없습니다. 참고용 설명입니다.
...
## 4. 로그인 (설정 화면의 '직접 로그인'과 동일, 콘솔 방식)

설정 화면의 '직접 로그인' 버튼 대신 콘솔에서 직접 실행할 수도 있습니다:
...
> 로그인 방법은 두 가지입니다. (1) 설정 화면에서 **아이디/비밀번호 저장** → 업로드 시 자동 로그인 시도,
> (2) **직접 로그인**으로 세션 저장(2단계 인증·캡차가 있으면 이 방법이 안정적).
> 자동 로그인이 막히면 자동으로 (2)로 안내합니다.

> ⚠ `browser_profiles/` 폴더에는 로그인 세션이 들어 있으니 절대 공유하지 마세요.

## 5. 영상을 어느 계정에 올릴지 지정하기
...
video_auto_uploader/
├── webui/
│   ├── app.py              # 브라우저 계정 설정 화면 (Flask)
│   └── templates/index.html
├── setup_accounts.bat      # 설정 화면 실행 (더블클릭)
├── main.py                 # 메인: 폴더 감시 + 계정 결정 + 업로드 오케스트레이션
├── setup_login.py          # 콘솔에서 계정×플랫폼 직접 로그인
├── config.json             # 계정 그룹 등록 및 모든 설정
├── run_uploader.bat        # 업로더 실행 (더블클릭)
├── uploader/
│   ├── common.py           # 설정/메타데이터/상태/브라우저 공용 코드
│   ├── credentials.py      # 아이디/비밀번호 암호화 저장 (Fernet)
│   └── platforms/
│       ├── facebook.py     # 페이스북 릴스
│       ├── instagram.py    # 인스타그램 릴스
│       ├── tiktok.py       # 틱톡 (TikTok Studio)
│       └── naver_clip.py   # 네이버 클립 (실험적)
├── secret.key              # 암호화 열쇠 (자동 생성, 커밋 금지)
├── credentials.enc         # 암호화된 로그인 정보 (자동 생성, 커밋 금지)
