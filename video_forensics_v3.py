        ...
        try:
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                if os.path.exists(downloaded_file):
                    ...
                    if metadata['fps']:
                        print(f"   🎬 프레임레이트: {metadata['fps']}fps")
                    if metadata['vbr']:
                        print(f"   📊 비디오 비트레이트: {metadata['vbr']}kbps")
                    if metadata['abr']:
                        print(f"   🎵 오디오 비트레이트: {metadata['abr']}kbps")
                    print(f"   🏷️ 포맷 ID: {metadata['format_id']}")
                    ...
                    try:
                        ...
                        print(f"   ✅ 실제 품질 검증: {actual_width}x{actual_height} @ {actual_fps:.1f}fps")
                        
                        # 품질 등급 표시
                        if actual_height >= 2160:
                            print(f"   🏆 품질 등급: 4K UHD (최상위)")
                        elif actual_height >= 1440:
                            print(f"   🥇 품질 등급: 2K QHD (매우 높음)")
                        elif actual_height >= 1080:
                            print(f"   🥈 품질 등급: Full HD (높음)")
            ...
            print("💡 다른 도구와 품질 차이가 없다면, 이미 최상위 품질입니다.")
            return None, None
...
def main():
    print("="*60)
    print("🔬 고급 영상 포렌식 분석 도구 v3 (최상위 품질 다운로드)")
    print("="*60)
    print("v3 개선사항:")
